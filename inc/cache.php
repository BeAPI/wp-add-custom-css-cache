...
class WASS_Cache_File {
	...
	private function get_cache_folder() {
		...
		$folder             = apply_filters( 'WASS_Cache/folder_name', '/cache/wass/' );
	...
	private function get_file_cache_name() {
		return sanitize_file_name( sprintf( '%d-wass.css', $this->blog_id ) );
	}
	...
	public function get_cache_url() {
		...
		return str_replace( 'wp/', '', home_url( $this->get_file_cache_relative_path() ) );
	}
	...
	public function set( $data ) {
		return $this->file_system->put_contents( $this->get_file_cache_path(), $data, 0775 );
	}
